...
def get_cb(viz_rn, f):
    if viz_rn is not None:
        viz_rn.set(v=f.smpl, background_image=np.dstack(
            (f.mask, f.mask, f.mask)))
...
def collision_obj(smpl, regs):
    sp = SphereCollisions(
        pose=smpl.pose, betas=smpl.betas, model=smpl, regs=regs)
    sp.no_hands = True
...
def init(frames, body_height, b2m, viz_rn):
    ...
    for w_prior, w_betas in weights:
        ...
        for i, f in enumerate(frames):
            if np.sum(f.keypoints[[0, 2, 5, 8, 11], 2]) > 3.:
                x0.extend(
                    [f.smpl.pose[list(range(21, 0)) + list(range(27, 30)) + list(range(36, 60))], f.smpl.trans])
...
def reinit_frame(frame, null_pose, nohands, viz_rn):
    ...
    if (np.sum(frame.pose_obj.r ** 2) > 625 or np.sum(frame.pose_prior_obj.r ** 2) > 75)\
        ...
        if nohands:
            x0.append(frame.smpl.pose[list(range(21)) +
                      list(range(27, 30)) + list(range(36, 60))])
        else:
            x0.append(frame.smpl.pose[list(range(21)) +
                      list(range(27, 30)) + list(range(36, 72))])
...
def fit_pose(frame, last_smpl, frustum, nohands, viz_rn):
    ...
    dist_i[dist_i < 0] = 0
    dist_i[dist_i > 50] = 50
    dist_o = cv2.distanceTransform(
        255 - np.uint8(frame.mask * 255), dst_type, 5)
    dist_o[dist_o > 50] = 50
    ...
    if nohands:
        x0 = [frame.smpl.pose[list(range(
            21)) + list(range(27, 30)) + list(range(36, 60))], frame.smpl.trans]
    else:
        x0 = [frame.smpl.pose[list(range(
            21)) + list(range(27, 30)) + list(range(36, 72))], frame.smpl.trans]
...
def main(keypoint_file, masks_file, camera_file, out, model_file, prior_file, resize,
    ...
    if display:
        debug_cam = ProjectPoints(
            v=base_smpl, t=camera.t, rt=camera.rt, c=camera.c, f=camera.f, k=camera.k)
        ...
        debug_rn = ColoredRenderer(
            camera=debug_cam, v=base_smpl, f=base_smpl.f, vc=debug_light, frustum=frustum)
    else:
        debug_rn = None
    ...
    def create_frame(i, smpl, copy=True):
        ...
        f.camera = ProjectPoints(
            v=f.smpl, t=camera.t, rt=camera.rt, c=camera.c, f=camera.f, k=camera.k)

        f.keypoints = np.array(
            keypoints[i]).reshape(-1, 3) * np.array([resize, resize, 1])
        f.J = joints_coco(f.smpl)
        f.J_proj = ProjectPoints(
            v=f.J, t=camera.t, rt=camera.rt, c=camera.c, f=camera.f, k=camera.k)
        ...
        f.pose_obj = (
            f.J_proj - f.keypoints[:, :2]) * f.keypoints[:, 2].reshape(-1, 1)
    ...
    num_init = 5
    indices_init = np.ceil(np.arange(num_init) *
                           num_frames * 1. / num_init).astype(np.int32)
    ...
    with h5py.File(out, 'w') as fp:
        last_smpl = None
        poses_dset = fp.create_dataset(
            "pose", (num_frames, 72), 'f', chunks=True, compression="lzf")
        trans_dset = fp.create_dataset(
            "trans", (num_frames, 3), 'f', chunks=True, compression="lzf")
        betas_dset = fp.create_dataset(
            "betas", (10,), 'f', chunks=True, compression="lzf")
